...
import sys
from utils import postprocess, csv_yield
...
parser.add_argument('--ground-data-csv-path', help = "Feed surface data from the csv file. See --generate-template-csv-file for format.")
parser.add_argument('--generate-template-csv-file', action = 'store_true', help = "Create a template csv file for ground data. Use with --dir argument.")
...
if __name__ == '__main__':
    if GENERATE_TEMPLATE_CSV_FILE:
        if not DIR:
            print('''No directory provided. Enter the path to the directory or the zip
        ...
        with open('ground_data_template.csv', 'w') as file:
            file.write('Datetime,Surface Pressure,Temperature,Dew Point,Wind Direction,Wind Speed\n')
    ...
    for file in iterate_files(sanitize_dir_input(DIR)):
        ...
        if GROUND_DATA_CSV:
            ...
            try:
                print(f'Continue with inserting surface data for file: {file}? (default: Y)')
